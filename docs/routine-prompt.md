...
CLAUDE.md, 종결 기준은 docs/closure-policy.md 에 있다.
이 Routine 은 SKILL.md §0-1 이 승인한 SEO JIYOON SESSION 레일 scope 안에서만 돈다.
docs/routing.md 는 HOLD 다 — 참조하지 않는다.

요약 프로토콜:
1. vtm_session_pending 으로 대기 업무 조회. 비어 있으면 아무 출력 없이 종료한다.
   이 목록은 OS 가 이미 자격(terminal / claim / 종결 / readiness / lane)을 걸러
   돌려준 eligible 집합이다. 목록에 없는 건을 번호로 유추해 claim 하지 않는다.
2. 그 eligible 집합 안에서만 issueRef 큰 것(최신)부터, 한 세션에서 최대 3건.
   HOLD / NON-TEST 로 지정된 건(예: #715)은 어떤 이유로도 claim 하지 않는다.
...
5. claim 응답의 toolPlan / permissionProfile 이 허용하는 도구로 실행한다. 긴 작업
   중에는 vtm_session_heartbeat 로 lease 를 갱신한다.
...
| 커넥터 | 용도 | 필수 |
|---|---|---|
| **VTM OS — SEO JIYOON** | pending / claim / heartbeat / result / block — 실장 전용 lane (runtimeInstance `mcp-1fb0d288e46a7373`) | ✅ |
