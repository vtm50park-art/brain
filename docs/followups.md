...
> ⚠️ **이 문서의 "상태" 줄은 등록 당시 스냅샷이다. 현행 상태가 아니다.**
> 각 항목의 상태를 인용하기 전에 `### 현행 상태` 블록이 있는지 먼저 보고,
> 없으면 코드·워크플로·배포를 직접 실측한다. 스냅샷을 현행으로 읽으면 안 된다.
> (근거: 2026-09-24 실측 — 서지윤 실장이 아래 §C 의 8/21 스냅샷
> "main·release 반영 및 Production deploy 금지 유지"를 현행으로 읽고
> 본부장께 "Telegram 이 막혀 있다"고 잘못 보고했다.)
...
**Telegram 자동 보고는 막혀 있지 않다. 라이브로 돌고 있다.**
위 "상태" 줄의 `main·release 반영 및 Production deploy 금지 유지` 는 2026-08-21
스냅샷이며 그 뒤 상황이 바뀌었다.

| 항목 | 현행 (실측) |
|---|---|
| `canonical-push.ts` | `release/vtm-os-v1` 에 반영됨 |
| 총괄과장 PASS → 종결 레일 | PR #730 병합(2026-09-23)으로 연결됨 |
| 직원 판정 → Telegram 보고 | `employee-verdict` 진입점으로 연결됨 — PR #744 병합 `040ffa8` |
| 실발송 실측 | `vtm-os-next#743` messageId 318(REWORK 경로) · 319(PASS 경로), 각 `attempts=1` |
| 남은 제약 | Production 승격은 본부장 지시로 **보류**. `release/vtm-os-v1` 까지만 |
| 자동 기상 | `employee-verdict` 는 수동 `workflow_dispatch` 전용 — 본부장 지시로 보류 |

아래 §C-1 이후 블록은 2026-08-21 기준 기록이므로 **역사 기록으로 읽는다.**
