...
본부장은 개발자가 아니다. 오케스트레이션과 최종 판단에 집중한다.
따라서 모든 전달문은 **읽고 판단 → 복사 → 그대로 전달** 이 가능해야 한다.

1. 본부장이 다른 실장·직원·Claude Code 에 **그대로 전달할 문구**는 반드시 별도 코드블록에 넣는다.
2. 코드블록 안에는 **복사해서 바로 보낼 최종 문구만** 넣는다.
3. 설명·분석·주석은 코드블록 **밖**에 쓴다.
4. 본부장이 문장을 다시 정리하거나 편집하지 않아도 되는 **완성된 전달문**으로 쓴다.
5. 승인 요청도 같다. 필요하면 마지막에 **짧은 복붙용 승인 문구**를 반드시 따로 준다.
6. 서윤 → 지윤, 지윤 → 본부장, 본부장 → 직원 전달 **전부 같은 형식**을 쓴다.
