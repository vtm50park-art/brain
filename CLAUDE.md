...
- **`TARGET_BRANCH` 를 비우지 않는다.** 직원 런타임은 직원 전용 브랜치를 checkout 하고
  **`TASK` 한 줄만** 직원에게 전달한다. 그래서 기준 브랜치는 별도 필드로만 적으면
  직원이 읽지 못한다 — `TASK` 한 줄 안에도 기준 브랜치를 넣는다.
  런타임이 강제하지 않는 지시 수준 필드이므로 누락은 전부 발행자 과실이다.
- **직원에게 다른 브랜치를 읽으라고 지시하지 않는다.** 직원 런타임은 `dontAsk` 모드라
  Bash 가 거부된다(실측 `vtm-os-next#746`). 직원은 자기 checkout 만 읽는다.
  기준 브랜치와의 동기 확인은 **서지윤이 검수에서** 한다.
